...
void G4VScoreWriter::DumpQuantityToFile(const G4String& psName,
  ...
  G4cout << "     " << unitValue << "   " << divisionAxisNames[0] << G4endl;
  // index order
  ...
  if(opt.find("sequence") != std::string::npos) {
    ofile << fNMeshSegments[0] << " " << fNMeshSegments[1] << " " << fNMeshSegments[2]  << G4endl;
  }
  ...
  for(int x = 0; x < fNMeshSegments[0]; x++) {
	...
	//G4cout << "     "  << idx << "   " << fNMeshSegments[0] << "  " << fNMeshSegments[1] << "   " << fNMeshSegments[2] <<  G4endl;
	//G4cout << "     " << x*fNMeshSegments[1]*fNMeshSegments[2] +y*fNMeshSegments[2]+z << G4endl;
	...
	if(value == score->end()) {
	  ...
	  ofile  << x << "\t" << y << "\t" << z << "\t" << (value->second->sum_wx())/unitValue << "\t" << value->second->n();
	}
